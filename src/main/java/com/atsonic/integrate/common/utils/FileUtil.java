...
public class FileUtil {
    ...
    public static boolean compareFileName(MultipartFile file, String compareFileName) {
        ...
        if (fileName.contains(".")){
            ...
            if (fileOtherName.equals(compareFileName)){
                return true;
            } else {
                return false;
            }
        } else {
            String fileOtherName = file.getOriginalFilename();
            if (fileOtherName.equals(compareFileName)){
                return true;
            } else {
                return false;
...
public static String getNamePart(String fileName) {
    ...
    //根据路径上传文件
    public static  void exportFile(String path, MultipartFile file) throws IOException {
        //1:使用File类创建一个要操作的文件路径
        File desFile = new File(path);
        if(!desFile.getParentFile().exists()){ //如果文件的目录不存在
            desFile.getParentFile().mkdirs(); //创建目录
        }
    ...
    //解压zip文件
    public static  void unZipFile(String source,String dest) throws IOException {
    ...
    public static void main(String[] args) {
        try {
            unZipFile("F:\\ai_voice_rms\\src\\main\\webApp\\aiwords\\555\\appId\\null\\1111.zip","F:\\ai_voice_rms\\src\\main\\webApp\\aiwords\\555\\appId\\null\\");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * @return
     */
    public static String loadFileToStr(InputStreamReader isr) throws IOException {

        StringBuilder fileStr = new StringBuilder();

        List<String> strs = loadTxtFile(isr);

        if (strs != null && strs.size() > 0) {

            for (String str : strs) {
                fileStr.append(str.trim());
            }
     ...
     * @return 文本文件中的每行的内容
     */
    private static List<String> loadTxtFile(InputStreamReader isr) throws IOException {

        BufferedReader br = null;
        List<String> lines = new ArrayList<>();//保存所有行数据
        try {
